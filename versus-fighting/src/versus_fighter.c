...
bool versus_fighter_init(VersusFighter *fighter, const USpriteDefinition *sprite_definition, u16 first_sprite, u8 controller_index, Vec2 position, bool facing_right) {
    ...
    *fighter = (VersusFighter){ 0 };
    ...
    fighter->character.actor.sprite.offset = (Vec2){ VERSUS_SPRITE_ANCHOR_X, VERSUS_SPRITE_ANCHOR_Y };
...
void versus_fighter_reset(VersusFighter *fighter, Vec2 position, bool facing_right) {
    ...
    fighter->character.actor.position = position;
    ...
    unsigned_sprite_set_flip_x(&fighter->character.actor.sprite, !facing_right);
    versus_fighter_set_state(fighter, VERSUS_FIGHTER_IDLE);
    versus_fighter_play_state_animation(fighter, VERSUS_FIGHTER_IDLE);
}
...
void versus_fighter_face_opponent(VersusFighter *fighter, const VersusFighter *opponent) {
    ...
    fighter->character.facing_right = fighter->character.actor.position.x <= opponent->character.actor.position.x ? 1u : 0u;
    unsigned_sprite_set_flip_x(&fighter->character.actor.sprite, fighter->character.facing_right == 0u);
}
...
bool versus_fighter_is_blocking(const VersusFighter *fighter, const UInputController *controller) {
    ...
    VersusFighterState state = versus_fighter_state(fighter);
    if (state == VERSUS_FIGHTER_KO || state == VERSUS_FIGHTER_HITSTUN) return false;
    UInputMask back = fighter->character.facing_right ? U_INPUT_BUTTON_LEFT : U_INPUT_BUTTON_RIGHT;
...
void versus_fighter_update(VersusFighter *fighter, const UInputController *controller, bool controls_enabled) {
    ...
    VersusFighterState state = versus_fighter_state(fighter);
    Vec2 *position = &fighter->character.actor.position;
    ...
    if (fighter->stun_frames > 0u) {
        ...
        if (fighter->stun_frames == 0u) versus_fighter_set_state(fighter, VERSUS_FIGHTER_IDLE);
        return;
    }
    ...
    } else if ((controller->state.pressed & U_INPUT_BUTTON_A) != 0u && versus_fighter_input_has_qcf(fighter)) {
    ...
    versus_state_machine_tick(&fighter->states);
}
...
void versus_fighter_apply_hit(VersusFighter *fighter, const VersusAttackDefinition *attack, s16 direction) {
    ...
    health->current_value = health->current_value > attack->damage ? (s16)(health->current_value - attack->damage) : 0;
    fighter->velocity.x = (s16)(direction * attack->pushback);
    fighter->stun_frames = attack->hitstun_frames;
    versus_fighter_set_state(fighter, health->current_value == 0 ? VERSUS_FIGHTER_KO : VERSUS_FIGHTER_HITSTUN);
}
